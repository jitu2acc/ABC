...
def create_personalized_prompt(user_profile, user_name):
    ...
    SPECIAL RESPONSE RULES:
    - If the user just says "Hi" or "Hello" (greeting only), respond with "tu Aandhar hai kya" in a playful, teasing way.
    - This is your signature greeting response - use it every time someone greets you with just "Hi" or "Hello".
    """
...
if not st.session_state['authenticated']:
    ...
    <div class="login-container">
        <h1 class="login-title">Welcome to Chat with A Kalhansh</h1>
        <p class="login-subtitle">Your personal AI companion A Kalhansh is waiting for you!</p>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown(f'<h1 class="main-title">Chat with A Kalhansh - Welcome {st.session_state["user_name"]}!</h1>', unsafe_allow_html=True)
...
if not st.session_state['current_session_id']:
    ...
    if st.session_state['history']:
        for msg in st.session_state['history']:
            if msg['role'] == 'user':
                ...
                <div style='display: flex; justify-content: flex-start; margin: 15px 0;'>
                    ...
                    '>
                        <div style='font-weight: 600; font-size: 12px; color: #e74c3c; margin-bottom: 4px;'>A Kalhansh</div>
                        {msg['content']}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        '>
            ...
            <p style='margin: 0; opacity: 0.9; font-size: 16px;'>A Kalhansh is waiting to hear from you!</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with col1:
            user_input = st.text_input(
                "Message", 
                key="chat_input", 
                placeholder="Say hi to A Kalhansh...", 
    ...
    if send_button and user_input:
        ...
        if (len(cleaned_input) == 0 or 
            ...
            st.warning("⚠️ Please type something to chat with A Kalhansh!")
            st.rerun()
        ...
        if st.session_state['history']:
            ...
            for msg in recent_messages:
                if msg['role'] == 'user':
                    ...
                    history_context.append(f"A Kalhansh: {msg['content']}")
    ...
    with st.expander("⚙️ Chat Settings & Personalization", expanded=False):
        ...
        selected_topics = st.multiselect(
            "Your Interests (A Kalhansh will reference these)",
            available_topics,
        ...
        custom_prompt = st.text_area(
            ...
            placeholder="Enter a custom prompt to define A Kalhansh's personality..."
        )
        ...
        if st.button("💾 Save Personalization Settings", type="primary"):
            try:
                ...
                if response.status_code == 200:
                    st.success("✅ Personalization settings saved! A Kalhansh will adapt to your preferences.")
                else:
        ...
        if current_profile:
            st.subheader("🔍 What A Kalhansh Has Learned About You")
